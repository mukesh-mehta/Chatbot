...
class RasaNLUHttpInterpreter(NaturalLanguageInterpreter):
    ...
    def _rasa_http_parse(self, text):
        """Send a text message to a running rasa NLU http server. Returns `None` on failure."""
        if not self.server:
            logger.error("Failed to parse text '{}' using rasa NLU over http. No rasa NLU server specified!".format(
                    text))
            return None
...
class RasaNLUInterpreter(NaturalLanguageInterpreter):
    def __init__(self, model_directory, config_file=None, lazy_init=False):
        ...
        if not lazy_init:
            self.interpreter = Interpreter.load(self.metadata, RasaNLUConfig(config_file))
        else:
    ...
    def parse(self, text):
        ...
        if self.lazy_init and self.interpreter is None:
            from rasa_nlu.model import Interpreter
            from rasa_nlu.config import RasaNLUConfig
            self.interpreter = Interpreter.load(self.metadata, RasaNLUConfig(self.config_file))
